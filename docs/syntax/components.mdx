...
- [`animations`](./syntax/animations) - Define timeline based animations
- [`consts`](./syntax/constants) - Local constants
- [`api`](./syntax/api) - Custom properties and slots (without `<enumdef>`s)
- [`styles`](./syntax/styles) - Local styles
- [`view`](./syntax/view) - Component structure
- [`previews`](./syntax/preview) - Preview configurations
...
- A custom [API](./syntax/api) can be defined
- Local [styles](./syntax/styles) can be defined, and global styles can be used
- Local [constants](./syntax/constants) can be defined, and global constants can be used
...
When a Component is created, it can extend another Component or Widget like [`<view extends="lv_slider">`](./syntax/view). If a Widget is extended, the component can use the Widget's API, for example:
...
Using Components in XML is very intuitive. The name of the Component can be used as an XML tag in the [`view`](./syntax/view) of other Components, [Screens](./syntax/screens), and [Widgets](./syntax/widgets). Component properties are simply XML attributes.
...
If loading XML at runtime is not needed, the [Editor](./editor) or the [CLI](./cli) can export C and H files from the XML files of the Components. The resulting code is completely self-sufficient and **the XML files are not needed anymore**. The resulting code is similar to what you could write manually.
...
If you need to access or modify values dynamically, it is recommended to use [data bindings via Subject](./syntax/data-bindings). You can also call these `..._create()` functions at any time from application code to create new Components on demand.
...
However, you can also define custom properties in the [`api` tag](./syntax/api). These properties can then be passed to any properties of the children by referencing them using `$`. For example:
...
See the [API documentation](./syntax/api) for more details and XML syntax reference for all the supported types.
