...
- **load** - Loads an already existing screen. After leaving the screen, it remains in memory, so all states are preserved. The screen to load mush have `<screen permanent="true"/>` 
- **create** - Creates the screen dynamically, and when leaving the screen, it is deleted, so all changes are lost (unless saved in subjects).   The screen to load mush have `<screen permanent="false"/>` (`false` is the default).
