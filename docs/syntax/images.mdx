...
The Editor convert PNG image files to C array that can be compiled into the editor:
...
As these kind of images are basically C code, you need to recompile the project int the editor to see them with the final color format. To show something on the screen before recompiling the Editor uses the PNG files as fallback which always has ARGB888 color format.
...
When the images are adding `globals.xml` they can be used like this on all places where
where a property has `image` type.
