...
<Callout>
The XML standard doesn't allow using some reserved characters in values. For example
`"`, `'`, `<`, `>`, and new line. 
This is it's invalid `value="I'm here"` as `'` needs to escaped as `"I&apos;m here"`.

The Editor show a little popup when you click on a string property, where you can write text in the normal way, and the editor will handle escaping.
...
Arrays can be defined in three ways depending on how the :
...
If there is an [`<api>`](./syntax/api) property like `<prop name="values" type="int[3]"/>` it can be used as `<my_widget values="10 33 55"/>` 

String items needs to wrapped into `'` because and item can be composed of multiple words to "space" alone can't be used as separation. For example `<prop name="list" type="string[NULL]"/>` it can be used as: `<my_widget values="'This is the first' 'The second' '3rd'"/>` 
