...
lv_obj_t * header_create(lv_obj_t * parent, const char * title)
{
    ...
    if (!style_inited) {
        ...
        lv_style_set_bg_color(&main_style, lv_color_hex(0x2d2d2d));
        lv_style_set_border_width(&main_style, 2);
        lv_style_set_border_side(&main_style, LV_BORDER_SIDE_BOTTOM);
        lv_style_set_border_color(&main_style, lv_color_hex(0xa2a2a2));
        lv_style_set_radius(&main_style, 0);
        lv_style_set_width(&main_style, lv_pct(100));
        lv_style_set_height(&main_style, 32);
        lv_style_set_pad_hor(&main_style, 8);
    ...
    lv_obj_bind_style(subtitle_1, &edited, 0, &hour_edited, 1);
    ...
    lv_obj_bind_style(subtitle_3, &edited, 0, &min_edited, 1);
